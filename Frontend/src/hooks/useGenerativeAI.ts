...
Certifique-se de que a resposta seja um JSON válido e nada mais.`;
      ...
      if (import.meta.env.DEV) {
        ...
        for (let i = 1; i <= 15; i++) {
          fallback.push({
            id: i,
            question: `[Modo Offline] Pergunta ${i} sobre ${subject}: Qual é um conceito importante?`,
            options: [
              `Resposta correta para pergunta ${i}`,
              `Opção incorreta A`,
              `Opção incorreta B`,
              `Opção incorreta C`
            ],
        ...
        // NÃO seta erro quando conseguimos gerar fallback
        setError(null);
      } else {
